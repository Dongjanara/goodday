...
ArrayList<WishlistItem> zzimList = (ArrayList<WishlistItem>) session.getAttribute("zzimList");
	%>
	<!-- ======================================================= -->
	<nav class="navbar navbar-expand-lg bg-body-tertiary fixed-top  ">
		<div class="container-fluid d-inline-flex displayb  ">
			...
			<form action="SelectArea.do "class="d-flex" role="search">
					...
					<ul class="navbar-nav mg-r d-flex">
						<%if(sid==null){%>		
	...
	<div class="row REcontainer" style="margin-top:100px;">
		...
		<div class="col-3 ">
			...
			<div class="list-group " style="max-width: 350px;">
				...
				<a href="02_jjim.jsp" class="list-group-item list-group-item-success active" aria-current="true" style="height: 60px">찜 목록 </a>
		...
		<div class="col-9 ">
			...
			<form class="REform ">
				<div class="titleRE tway1">Wish List</div>
				...
				<table class="table" style="text-align: center;">
					...
					<c:if test="${empty zzimList}">
   						 <tr>
       				 		<td colspan="4">찜 목록이 비었습니다.</td>
  						 </tr>
					</c:if>
					
					 <c:forEach var="vo1" items="${zzimList}" varStatus="loop">  
        				<tr>	
						<td>${loop.index + 1}</td>
						<td>${vo1.soo }</td>
						<td><a href="view.do?name=${vo1.name}"> [${vo1.area}]  ${vo1.name}</a></td>
						<c:set var="amountWithoutComma" value="${fn:replace(vo1.price, ',', '')}" />
           		 <c:set var="priceInt" value="${amountWithoutComma ne '' ? 
           		 					Integer.parseInt(amountWithoutComma) : 0}" />                      		 
          		  <td>              
<%--               		 <fmt:formatNumber type="currency" currencyCode="KRW"
              		 							 value="${priceInt}" pattern="###,###원"/> --%>
           		 </td>
						</tr>
						</c:forEach>			
					</tbody>
